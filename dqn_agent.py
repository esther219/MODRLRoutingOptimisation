...
class QNetwork(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed):
        ...
        layers = [ 10*4*state_size, 10*8*state_size,10*4*state_size ]
        self.fp1 = nn.Linear(state_size, layers[0])
        self.fp2 = nn.Linear(layers[0], layers[1])
        self.fp3 = nn.Linear(layers[1], layers[2])
        self.head_values = nn.Linear(layers[2], 1)
        self.head_advantages = nn.Linear(layers[2], action_size)
